...
app = FastAPI(title="Football Live Dashboard", version="3.1.0")
...
STATUS_MAP = {
    "inprogress": "LIVE", "1h": "LIVE", "2h": "LIVE",
    "ht": "LIVE", "et": "LIVE", "pen": "LIVE",
    "notstarted": "SCHEDULED",
    "finished": "FINISHED", "postponed": "FINISHED", "canceled": "FINISHED",
}
...
def _flag(country: str) -> str:
    flags = {
        ...
        "italy": "🇮🇹", "france": "🇫🇷", "brazil": "🇧🇷",
        "portugal": "🇵🇹", "netherlands": "🇳🇱", "europe": "⭐", "world": "🌍",
    }
...
def _normalize_match(raw: dict) -> dict:
    ...
    return {
        "id": mid, "api_id": mid,
        "league": league_name, "league_logo": _flag(country),
        "home": {"name": home_t.get("name", "Casa"),
                 "short": home_t.get("shortName", home_t.get("name", "HOM"))[:3].upper(),
                 "logo": "🟡"},
        "away": {"name": away_t.get("name", "Visitante"),
...
def _empty_stats() -> dict:
    return {
        "possession": [50, 50], "shots_on_target": [0, 0], "shots_total": [0, 0],
...
STAT_KEY_MAP = {
    "Ball possession": "possession", "Shots on goal": "shots_on_target",
    "Total shots": "shots_total", "Fouls": "fouls",
...
# ---------------------------------------------------------------------------
# FootAPI7 — rotas corretas
# /api/matches/live                      → jogos ao vivo
# /api/football/matches/{day}/{month}/{year} → jogos por data
# /api/match/{id}/statistics             → stats de uma partida
# ---------------------------------------------------------------------------
...
async def _fetch_today_matches() -> list[dict]:
    """Usa a rota correta: /api/football/matches/{day}/{month}/{year}"""
    now = datetime.now(timezone.utc)
    day, month, year = now.day, now.month, now.year
    url = f"{API_BASE}/api/football/matches/{day}/{month}/{year}"
    try:
        r = await get_http().get(url, headers=HEADERS)
        r.raise_for_status()
        data   = r.json()
        events = data.get("events") or data.get("matches") or []
        log.info(f"FootAPI7 today ({day}/{month}/{year}): {len(events)} jogos")
        return [_normalize_match(e) for e in events]
    except Exception as exc:
...
MOCK_MATCHES: list[dict[str, Any]] = [
    {"id":"m1","api_id":"m1","league":"LaLiga","league_logo":"🇪🇸",
     "home":{"name":"Real Madrid","short":"RMA","logo":"⚪"},"away":{"name":"Barcelona","short":"BAR","logo":"🔵"},
     "score":{"home":1,"away":0},"minute":34,"status":"LIVE","kickoff":None,
     "events":[{"minute":12,"type":"goal","team":"home","player":"Vini Jr."}],
     "stats":{"possession":[58,42],"shots_on_target":[5,3],"shots_total":[9,6],"fouls":[8,11],"yellow_cards":[1,2],"red_cards":[0,0],"corners":[4,3],"offsides":[2,1]}},
    {"id":"m2","api_id":"m2","league":"Brasileirão","league_logo":"🇧🇷",
     "home":{"name":"Flamengo","short":"FLA","logo":"🔴"},"away":{"name":"Palmeiras","short":"PAL","logo":"🟢"},
     "score":{"home":0,"away":0},"minute":67,"status":"LIVE","kickoff":None,
     "events":[],
     "stats":{"possession":[45,55],"shots_on_target":[2,4],"shots_total":[7,8],"fouls":[14,10],"yellow_cards":[3,1],"red_cards":[0,0],"corners":[5,6],"offsides":[3,2]}},
    {"id":"m3","api_id":"m3","league":"Premier League","league_logo":"🏴󠁧󠁢󠁥󠁮󠁧󠁿",
     "home":{"name":"Manchester City","short":"MCI","logo":"🩵"},"away":{"name":"Arsenal","short":"ARS","logo":"🔴"},
     "score":{"home":2,"away":2},"minute":78,"status":"LIVE","kickoff":None,
     "events":[{"minute":23,"type":"goal","team":"home","player":"Haaland"},{"minute":41,"type":"goal","team":"away","player":"Saka"},{"minute":55,"type":"goal","team":"away","player":"Martinelli"},{"minute":78,"type":"goal","team":"home","player":"De Bruyne"}],
     "stats":{"possession":[61,39],"shots_on_target":[7,5],"shots_total":[14,9],"fouls":[9,13],"yellow_cards":[2,3],"red_cards":[0,1],"corners":[8,4],"offsides":[1,3]}},
    {"id":"m4","api_id":"m4","league":"Champions League","league_logo":"⭐",
     "home":{"name":"PSG","short":"PSG","logo":"🔵"},"away":{"name":"Bayern","short":"BAY","logo":"🔴"},
     "score":{"home":0,"away":0},"minute":0,"status":"SCHEDULED","kickoff":"21:00","events":[],
     "stats":{"possession":[50,50],"shots_on_target":[0,0],"shots_total":[0,0],"fouls":[0,0],"yellow_cards":[0,0],"red_cards":[0,0],"corners":[0,0],"offsides":[0,0]}},
    {"id":"m5","api_id":"m5","league":"Serie A","league_logo":"🇮🇹",
     "home":{"name":"Inter Milan","short":"INT","logo":"⚫"},"away":{"name":"AC Milan","short":"MIL","logo":"🔴"},
     "score":{"home":3,"away":1},"minute":90,"status":"FINISHED","kickoff":None,
     "events":[{"minute":8,"type":"goal","team":"home","player":"Lautaro"},{"minute":33,"type":"goal","team":"away","player":"Leão"},{"minute":61,"type":"goal","team":"home","player":"Thuram"},{"minute":88,"type":"goal","team":"home","player":"Lautaro"}],
     "stats":{"possession":[52,48],"shots_on_target":[9,4],"shots_total":[17,8],"fouls":[11,15],"yellow_cards":[2,4],"red_cards":[0,0],"corners":[7,3],"offsides":[2,4]}},
]
...
async def _simulate_tick() -> None:
    for match in [m for m in _matches if m["status"] == "LIVE"]:
        ...
        if match["minute"] >= 90:
            match["status"] = "FINISHED"; continue
        s = match["stats"]
        ...
        if random.random() < 0.08 and mid in MOCK_PLAYERS:
            ...
            match["score"][side] += 1
            match["events"].append({"minute": match["minute"], "type": "goal", "team": side,
...
async def api_status() -> dict[str, Any]:
    return {"source": "real" if _using_real_api else "mock",
